...
suit_t flush_suit(deck_t* hand) {
    ...
    for (int i = 0; i < hand->n_cards; i++) {
        suits[hand->cards[i]->suit - SPADES]++;
    }
    for (int i = 0; i < 4; i++) {
        if (suits[i] >= 5) {
            return SPADES + i;
...
int n_straight(deck_t* hand, size_t index, suit_t fs, int n) {
    if (hand->n_cards - index < n || (fs != NUM_SUITS && hand->cards[index]->suit != fs)) {
        return 0;
    }
    int count = 1;
    card_t** cards = hand->cards;
    unsigned prev = cards[index]->value;
    for (size_t i = index + 1; i < hand->n_cards; i++) {
        if (fs == NUM_SUITS || cards[i]->suit == fs) {
            unsigned temp = cards[i]->value;
            if (temp < prev - 1) {
                break;
            }
            else if (temp == prev - 1) {
...
int is_straight_at(deck_t* hand, size_t index, suit_t fs) {
    int haveStraight = n_straight(hand, index, fs, 5);
...
hand_eval_t build_hand_from_match(deck_t * hand,
    ...
    for (int i = 0; i < 5; i++) {
        if (i < n) {
            ...
            for (int j = prev; j < hand->n_cards; j++) {
